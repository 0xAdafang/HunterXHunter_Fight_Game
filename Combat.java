...
public class Combat 
{
    ...
    public boolean lancerTour(int choixJoueur, int choixAdversaire) 
    {
        ...
        //ajout capacité Ultime

        if(choixJoueur == 4 && joueur.getCapaciteUltime().estDisponible())
        {
            int degatsUltimeJoueur = joueur.getCapaciteUltime().activer(joueur, adversaire, 0, coupCritiqueJoueur);
            adversaire.setVie(adversaire.getVie() - degatsUltimeJoueur);
            System.out.println(adversaire.getNom() + " a maintenant " + adversaire.getVie() + " point de vie !");
        }

        if(choixAdversaire == 4 && adversaire.getCapaciteUltime().estDisponible())
        {
            int degatUltimeAdversaire = adversaire.getCapaciteUltime().activer(adversaire, joueur, 0, coupCritiqueAdversaire);
            joueur.setVie(joueur.getVie() - degatUltimeAdversaire);
            System.out.println(joueur.getNom() + " a maintenant " + joueur.getVie() + " point de vie !");
        }
        ...
        // Augmenter le compteur de tours pour la capacité ultime
